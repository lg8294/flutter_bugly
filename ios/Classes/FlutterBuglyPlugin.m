...
- (void)handleMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result {
    if ([@"initBugly" isEqualToString:call.method]) {
        ...
        if (!b) {
            ...
            [Bugly startWithAppId:appId developmentDevice:isDebug.boolValue config:config];
            NSLog(@"Bugly appId: %@", appId);

            NSDictionary *dict = @{
                    @"message": @"Bugly 初始化成功", @"appId": appId, @"isSuccess": @YES
            };
            ...
            NSDictionary *dict = @{
                    @"message": @"Bugly appId不能为空", @"isSuccess": @NO
            };
            NSData *jsonData = [NSJSONSerialization dataWithJSONObject:dict options:NSJSONWritingPrettyPrinted error:nil];
            NSString *json = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
